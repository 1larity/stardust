...
public class Stardust3d extends Game {
	...
	public static void refreshActors() {
		...
		Stardust3d.MyDataOp.getLocalPlayers(Stardust3d.myCharacter.getX(),
				Stardust3d.myCharacter.getY(), Stardust3d.myCharacter.getZ(),
				false);
	...
	public static void refreshChat(int selectedChannel, boolean serverMessages) {
		...
		if (chatListReset) {
			...
			if (selectedChannel == 8) {
				MyDataOp.getchat(-Stardust3d.myCharacter.getUid(), true,
						serverMessages);
			} else {
			...
			if (selectedChannel == 8) {
				MyDataOp.getchat(-Stardust3d.myCharacter.getUid(), false,
						serverMessages);
			} else {
		...
		if (!serverMessages) {
			...
			String shortChatlines = Stardust3d.chatChannelName + "\n";
			try {
